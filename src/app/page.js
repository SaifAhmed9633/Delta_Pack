  ...
  return (
    <>
      ...
      <section className="relative min-h-[90vh] w-full flex flex-col md:flex-row items-center overflow-hidden">
        ...
        {/* Left: Text */}
        <div className="w-full md:w-1/2 px-4 sm:px-6 md:pl-12 lg:pl-24 z-30 order-2 md:order-1 flex flex-col justify-center h-full pt-8 md:pt-0">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
            ...
            <h1 className="font-oswald text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold leading-[0.9] mb-4 sm:mb-6 tracking-tight">
              PRECISION <br />
              <span className="stroke-text opacity-40 text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-800" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.5)' }}>CONTAINERS</span>
            </h1>
            <p className="text-gray-400 max-w-md text-sm leading-relaxed mb-8 border-l-2 border-green-500/50 pl-4">
              تكنولوجيا تصنيع ألمانية بأيادٍ مصرية. نقدم عبوات ورقية بمواصفات عالمية (16oz Bowl) تضمن المتانة وعدم التسريب لمنتجك.
            </p>
            <div className="flex flex-wrap gap-3 sm:gap-4">
              <a href="#contact" className="bg-white text-black px-5 sm:px-8 py-2.5 sm:py-3 font-oswald font-bold text-xs sm:text-sm tracking-wider hover:scale-105 transition-transform shadow-[0_0_20px_rgba(255,255,255,0.2)] flex items-center gap-2">START PROJECT</a>
              <a href="#models" className="group border border-white/20 px-5 sm:px-8 py-2.5 sm:py-3 font-oswald font-bold text-xs sm:text-sm tracking-wider hover:bg-white hover:text-black transition-all flex items-center gap-2">VIEW SPECS<span className="group-hover:translate-x-1 transition-transform">→</span></a>
            </div>
          </motion.div>
        </div>
        ...
        <div className="w-full md:w-1/2 h-[50vh] md:h-[100vh] relative z-20 order-1 md:order-2 bg-gradient-to-b from-transparent via-transparent to-black/20">
          <div className="w-full h-full cursor-grab active:cursor-grabbing"><CupScene /></div>
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1 }}
            ...
            <div className="flex justify-between items-center text-[10px] font-mono text-green-500 mb-4 tracking-widest"><span>● LIVE SPECS</span><span className="border border-green-500/30 px-1 rounded">ISO 9001</span></div>
            <div className="space-y-3 border-t border-white/10 pt-3 text-xs font-mono">
              {[['TYPE','16oz BOWL'],['TOP DIA','114 MM'],['HEIGHT','72 MM'],['VOLUME','440 ML']].map(([k,v]) => (
                <div key={k} className="flex justify-between items-center"><span className="text-gray-500">{k}</span><span className="text-white font-bold">{v}</span></div>
              ))}
            </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
          className="absolute bottom-8 left-1/2 -translate-x-1/2 hidden md:flex flex-col items-center gap-2 z-30 mix-blend-difference">
      ...
      {/* Marquee */}
      <div className="relative z-20 bg-[#0a0a0a] border-y border-white/5"><Marquee /></div>
